...
if ($viewpage == 1) {
    ...
    if (isset($_REQUEST['submit'])) {
        ...
        // LabsMobile
          ...
          $check = send_sms_labsmobile($number, $msg);
          if (stripos($check, "successfully") === FALSE) {
            $checkdesc = "Unknown";
            if(stripos($check, "<message>") !== FALSE) {
              $initpos = stripos($check, "<message>") + 9;
              $endpos = stripos($check, "</message>") + 6;
              $checkdesc = substr($check, $initpos, $endpos - $initpos);
            }
            echo "<script>alert('Message(s) not sent due to an error: $checkdesc')</script>";
          } else {
            $sent_count = sizeof($number);
            echo "<script>alert('$sent_count message(s) successfully sent!')</script>";
          }
      }
    }
